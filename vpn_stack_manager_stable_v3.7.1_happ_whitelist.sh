...
set_manager_update_url() {
  ...
  save_env
  echo -e "${C_GREEN}URL обновления сохранён:${C_RESET} ${MANAGER_UPDATE_URL}"
}
...
1. Для Gemini / Google AI через x-ui:
   ...
   - Скрипт поставит WARP, включит локальный SOCKS5 и покажет, что добавить в x-ui.
   - В x-ui сначала создай outbound WARP, затем добавь routing-правила.
   - Правила WARP должны стоять ВЫШЕ direct / blocked / balancer.

2. Для отдельного прокси на сервере:
   - Открой «Прокси 3proxy».
   - Там находятся установка, выбор типа (HTTP/HTTPS, SOCKS5, SOCKS4), авторизация и данные подключения.

3. Для MTProto Proxy:
   - Открой «MTProto Proxy».
   - Сначала настрой порты и установи прокси.
   - Затем возьми готовую ссылку для Telegram и при желании включи автопочинку.

4. Для уведомлений и Telegram-бота:
...
quickstart_proxy() {
  echo -e "${C_CYAN}Сценарий: отдельный прокси 3proxy.${C_RESET}"
  install_socks5_proxy || true
  echo
  show_socks5_connection_info
}

quickstart_mtproto() {
  echo -e "${C_CYAN}Сценарий: быстрый запуск MTProto Proxy.${C_RESET}"
  install_component "$MTPROTO_COMP" || true
  echo
  show_mtproto_links || true
}
...
quick_install_all() {
  ...
  echo
  echo -e "${C_DIM}MTProto и 3proxy вынесены в отдельные разделы, потому что они нужны не на каждом сервере.${C_RESET}"
}

quickstart_menu() {
  while true; do
    show_header
    echo -e "${C_GRAY}Сценарии: быстрые действия без поиска по меню. Выбирай сценарий под задачу, а потом при необходимости переходи в профильный раздел.${C_RESET}"
    echo
    menu_line "1" "✨" "$C_GREEN" "Минимум для Gemini / x-ui — WARP + локальный SOCKS + подсказки по x-ui"
    menu_line "2" "🧦" "$C_CYAN" "Быстро поднять отдельный прокси 3proxy"
    menu_line "3" "📨" "$C_GREEN" "Быстро поднять MTProto Proxy"
    menu_line "4" "🛟" "$C_BLUE" "Поставить базовую автоматизацию: watchdog, status, daily report"
    menu_line "5" "🚀" "$C_YELLOW" "Полная установка core-набора менеджера"
    menu_line "6" "🗺" "$C_MAGENTA" "Показать карту разделов и порядок действий"
    menu_line "0" "↩" "$C_GRAY" "Назад"
    read -r -p $'\033[36mВыбери:\033[0m ' c
    case "$c" in
      1) quickstart_warp_xui; pause ;;
      2) quickstart_proxy; pause ;;
      3) quickstart_mtproto; pause ;;
      4) quickstart_base_automation; pause ;;
      5) quick_install_all; pause ;;
      6) show_getting_started_map; pause ;;
      0) break ;;
      *) echo -e "${C_RED}Неверный выбор.${C_RESET}"; pause ;;
    esac
  done
...
show_logs_menu() {
  while true; do
    ...
    menu_line "3" "📜" "$C_MAGENTA" "Показать лог MTProto watchdog"
    ...
    menu_line "7" "📘" "$C_BLUE" "Показать journal MTProto"
    menu_line "8" "📘" "$C_BLUE" "Показать journal 3proxy"
...
socks5_runtime_menu() {
  while true; do
    show_header
    echo -e "${C_GRAY}Управление 3proxy: здесь только сервисные действия и проверки. Тип, авторизация и данные подключения вынесены в основной раздел прокси.${C_RESET}"
    echo
    menu_line "1" "▶️" "$C_GREEN" "Запустить прокси"
    menu_line "2" "⏹" "$C_RED" "Остановить прокси"
    menu_line "3" "🔄" "$C_YELLOW" "Перезапустить прокси"
    menu_line "4" "📊" "$C_BLUE" "Статус прокси"
    menu_line "5" "🧪" "$C_CYAN" "Проверить локальный протокол прокси"
    menu_line "0" "↩" "$C_GRAY" "Назад"
    read -r -p $'\033[36mВыбери:\033[0m ' c
    case "$c" in
      1) start_component "$SOCKS5_COMP"; pause ;;
      2) stop_component "$SOCKS5_COMP"; pause ;;
      3)
         run_cmd systemctl restart "$(service_name_for "$SOCKS5_COMP")"
         if proxy_runtime_check; then
           echo "Прокси 3proxy перезапущен и отвечает корректно."
         ...
         pause ;;
      4) status_socks5_proxy; pause ;;
      5) proxy_runtime_check; pause ;;
      0) break ;;
      *) echo -e "${C_RED}Неверный выбор.${C_RESET}"; pause ;;
    esac
  done
...
socks5_menu() {
  while true; do
    show_header
    echo -e "${C_GRAY}Прокси 3proxy: установка из исходников, выбор типа (HTTP/HTTPS, SOCKS5, SOCKS4), авторизация и данные подключения. Порт подбирается автоматически.${C_RESET}"
    echo
    menu_line "1" "🧦" "$C_GREEN" "Установить / обновить прокси 3proxy"
    menu_line "2" "⚙️" "$C_YELLOW" "Изменить тип, авторизацию и учётные данные"
    menu_line "3" "🔐" "$C_CYAN" "Показать данные для подключения и подсказки"
    menu_line "4" "▶️" "$C_BLUE" "Управление сервисом и локальные проверки"
    menu_line "5" "🗑" "$C_RED" "Удалить прокси 3proxy"
    menu_line "0" "↩" "$C_GRAY" "Назад"
    read -r -p $'\033[36mВыбери:\033[0m ' c
    case "$c" in
      1) install_socks5_proxy; pause ;;
      2)
        ...
        pause ;;
      3) show_socks5_connection_info; pause ;;
      4) socks5_runtime_menu ;;
      5) remove_socks5_proxy; pause ;;
      0) break ;;
      *) echo -e "${C_RED}Неверный выбор.${C_RESET}"; pause ;;
    esac
  done
...
mtproto_runtime_menu() {
  while true; do
    show_header
    echo -e "${C_GRAY}MTProto: основной сервис, порты, ссылки и secret. Здесь находится всё, что нужно для реального подключения клиентов.${C_RESET}"
    echo
    ...
    menu_line "8" "♻️" "$C_CYAN" "Пересоздать secret и ссылку MTProto"
    menu_line "0" "↩" "$C_GRAY" "Назад"
    read -r -p $'\033[36mВыбери:\033[0m ' c
    case "$c" in
      ...
      8) regenerate_mtproto_secret; pause ;;
      0) break ;;
      *) echo -e "${C_RED}Неверный выбор.${C_RESET}"; pause ;;
    esac
  done
...
mtproto_automation_menu() {
  while true; do
    show_header
    echo -e "${C_GRAY}Автоматизация MTProto: ручной ремонт, автопочинка и ежедневная самопроверка. Включай это после того, как сам прокси уже установлен и работает.${C_RESET}"
    echo
    menu_line "1" "🩺" "$C_GREEN" "Починить MTProto Proxy"
    menu_line "2" "🛟" "$C_GREEN" "Включить автовосстановление MTProto"
    menu_line "3" "🛑" "$C_RED" "Выключить автовосстановление MTProto"
    menu_line "4" "📊" "$C_BLUE" "Статус автовосстановления MTProto"
    menu_line "5" "🗓" "$C_GREEN" "Включить ежедневную самопроверку MTProto"
    menu_line "6" "🗓" "$C_RED" "Выключить ежедневную самопроверку MTProto"
    menu_line "7" "📊" "$C_BLUE" "Статус ежедневной самопроверки MTProto"
    menu_line "0" "↩" "$C_GRAY" "Назад"
    read -r -p $'\033[36mВыбери:\033[0m ' c
    case "$c" in
      1) mtproto_repair; pause ;;
      2) enable_mtproto_recovery; pause ;;
      3) disable_mtproto_recovery; pause ;;
      4) status_mtproto_recovery; pause ;;
      5) enable_mtproto_daily_check; pause ;;
      6) disable_mtproto_daily_check; pause ;;
      7) status_mtproto_daily_check; pause ;;
      0) break ;;
      *) echo -e "${C_RED}Неверный выбор.${C_RESET}"; pause ;;
    esac
  done
...
mtproto_stats_menu() {
  while true; do
    show_header
    echo -e "${C_GRAY}История MTProto: прошлые автопочинки, счётчики, переключения на резервный порт и очистка накопленной статистики.${C_RESET}"
    echo
    menu_line "1" "🕘" "$C_BLUE" "Показать историю MTProto"
    menu_line "2" "📈" "$C_BLUE" "Показать счётчики MTProto"
    menu_line "3" "🧹" "$C_YELLOW" "Сбросить историю и счётчики MTProto"
    menu_line "0" "↩" "$C_GRAY" "Назад"
    read -r -p $'\033[36mВыбери:\033[0m ' c
    case "$c" in
      1) show_mtproto_history; pause ;;
      2) show_mtproto_counters; pause ;;
      3) reset_mtproto_counters; pause ;;
      0) break ;;
      *) echo -e "${C_RED}Неверный выбор.${C_RESET}"; pause ;;
    esac
  done
...
mtproto_menu() {
  while true; do
    show_header
    echo -e "${C_GRAY}MTProto Proxy: отдельный раздел для сервиса Telegram. Установка, ссылки, автоматизация и история разнесены по подменю, чтобы не смешивать разные задачи.${C_RESET}"
    echo
    menu_line "1" "📨" "$C_GREEN" "Установка и основное управление MTProto"
    menu_line "2" "🛟" "$C_BLUE" "Автоматизация и самопроверка MTProto"
    menu_line "3" "📈" "$C_CYAN" "История и счётчики MTProto"
    menu_line "4" "🗑" "$C_RED" "Удалить MTProto Proxy"
    menu_line "0" "↩" "$C_GRAY" "Назад"
    read -r -p $'\033[36mВыбери:\033[0m ' c
    case "$c" in
      1) mtproto_runtime_menu ;;
      2) mtproto_automation_menu ;;
      3) mtproto_stats_menu ;;
      4) remove_mtproto; pause ;;
      0) break ;;
      *) echo -e "${C_RED}Неверный выбор.${C_RESET}"; pause ;;
    esac
  done
...
reports_menu() {
  while true; do
    show_header
    echo -e "${C_GRAY}Отчёты и status: vpn-status показывает состояние стека, daily report отправляет сводку в Telegram, если токен и Chat ID уже настроены.${C_RESET}"
    echo
    ...
    case "$c" in
      ...
      7)
         local comp
         for comp in "$OPTIMIZE_COMP" "$WARP_WATCHDOG_COMP" "$XRAY_WATCHDOG_COMP" "$STATUS_COMP" "$DAILY_COMP" "$TG_CONTROL_COMP" "$MTPROTO_WATCHDOG_COMP" "$MTPROTO_DAILY_COMP" "$LOGROTATE_COMP"; do
           echo "---- $comp ----"
...
diagnostics_menu() {
  while true; do
    show_header
    echo -e "${C_GRAY}Диагностика: быстрые проверки живости сервисов, локальных прокси и нагрузки сервера. Для подробных причин сбоев переходи в журналы.${C_RESET}"
    echo
    menu_line "1" "🧪" "$C_CYAN" "Проверить всё"
    menu_line "2" "🤖" "$C_BLUE" "Проверить Telegram"
    menu_line "3" "🧦" "$C_GREEN" "Проверить локальный WARP SOCKS"
    menu_line "4" "🧦" "$C_GREEN" "Проверить локальный 3proxy"
    menu_line "5" "🌍" "$C_CYAN" "Проверить WARP trace через SOCKS"
    menu_line "6" "⚙️" "$C_BLUE" "Проверить x-ui / Xray"
    menu_line "7" "⏱" "$C_YELLOW" "Проверить timers/services"
    menu_line "8" "💾" "$C_MAGENTA" "Проверить диск / RAM / load"
    menu_line "9" "📊" "$C_BLUE" "Показать vpn-status"
    menu_line "10" "📜" "$C_MAGENTA" "Открыть раздел журналов"
    menu_line "0" "↩" "$C_GRAY" "Назад"
    read -r -p $'\033[36mВыбери:\033[0m ' c
    case "$c" in
      1)
        ...
        echo -e "Сервис 3proxy: $(fmt_status "$(safe_is_active "$(service_name_for "$SOCKS5_COMP")")")"
      ...
      5)
        ...
        pause ;;
      6) systemctl status "$XRAY_SERVICE_NAME" --no-pager 2>/dev/null || true; pause ;;
      7) systemctl list-timers --all | grep 'vpn-tools-' || echo "Таймеры менеджера не найдены."; pause ;;
      8) df -h /; echo; free -m; echo; uptime; pause ;;
      9)
...
- Backup: ${BACKUP_DIR}
- Конфиг 3proxy: ${SOCKS5_CFG_FILE}
- Директория MTProto: ${MTPROTO_DIR}
EOF_PATHS
}
...
remove_everything_except_manager() {
  remove_automation_components
  if have_mtproto; then
    remove_component "$MTPROTO_COMP" >/dev/null 2>&1 || true
    rm -rf "$MTPROTO_DIR"
    rm -f "$MTPROTO_INFO_FILE" "$MTPROTO_STATS_FILE" "$MTPROTO_HISTORY_FILE"
  fi
  if have_socks5_proxy; then
    remove_component "$SOCKS5_COMP" >/dev/null 2>&1 || true
    rm -f "$SOCKS5_CFG_FILE"
    rm -rf /usr/local/3proxy "$SOCKS5_SRC_DIR"
  fi
  if have_warp; then
...
remove_menu() {
  while true; do
    show_header
    echo -e "${C_GRAY}Удаление: сначала удаляй отдельный компонент, если точно знаешь, что больше он не нужен. Массовое удаление оставь на конец.${C_RESET}"
    echo
    menu_line "1" "🗑" "$C_RED" "Удалить только WARP"
    menu_line "2" "🗑" "$C_RED" "Удалить только прокси 3proxy"
    menu_line "3" "🗑" "$C_RED" "Удалить только MTProto Proxy"
    menu_line "4" "🧹" "$C_YELLOW" "Удалить только automation-компоненты и бота"
    menu_line "5" "💣" "$C_RED" "Удалить весь стек, но оставить менеджер"
    menu_line "6" "☠" "$C_RED" "Удалить всё, включая сам менеджер"
    menu_line "0" "↩" "$C_GRAY" "Назад"
    read -r -p $'\033[36mВыбери:\033[0m ' c
    case "$c" in
      1) remove_warp; pause ;;
      2) remove_socks5_proxy; pause ;;
      3) remove_mtproto; pause ;;
      4) remove_automation_components; echo "Automation-компоненты и бот удалены."; pause ;;
      5)
...
main_menu() {
  ...
  while true; do
    show_header
    echo -e "${C_GRAY}Главное меню сгруппировано по сценариям: сначала быстрый старт, затем профильные разделы сервисов, затем автоматизация, диагностика и обслуживание.${C_RESET}"
    echo
    menu_line "1" "🚀" "$C_GREEN" "Сценарии и быстрый старт"
    menu_line "2" "🛡" "$C_CYAN" "WARP и интеграция в x-ui"
    menu_line "3" "🧦" "$C_CYAN" "Прокси 3proxy"
    menu_line "4" "📨" "$C_GREEN" "MTProto Proxy"
    menu_line "5" "🤖" "$C_BLUE" "Telegram и уведомления"
    menu_line "6" "🩺" "$C_YELLOW" "Мониторинг и автоматизация"
    menu_line "7" "🔎" "$C_BLUE" "Диагностика и журналы"
    menu_line "8" "🗂" "$C_MAGENTA" "Обслуживание: backup, update, remove"
    menu_line "9" "✅" "$C_GREEN" "Обновить белый список Happ"
    ...
    case "$choice" in
      ...
      7) diagnostics_menu ;;
      8) maintenance_menu ;;
      9) update_happ_whitelist_page; pause ;;
      0) exit 0 ;;
      *) echo -e "${C_RED}Неверный выбор.${C_RESET}"; pause ;;
    esac
  done
