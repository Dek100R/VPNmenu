...
Добавь этот outbound в панели x-ui вручную.
...
show_warp_xui_routing_snippets() {
  cat <<'EOF_WARP_ROUTING'
Примеры routing rules для x-ui. Добавляй вручную в панели.

Вариант 1 — только OpenAI / Anthropic / Perplexity:
{
  "type": "field",
  "outboundTag": "warp",
  "domain": [
...
Вариант 3 — общий geosite для AI, если он есть в твоих списках:
{
  "type": "field",
  "outboundTag": "warp",
  "domain": [
...
После добавления правил перезапусти Xray в панели x-ui.
EOF_WARP_ROUTING
}
...
show_warp_xui_manual_steps() {
  ...
  port="$(warp_proxy_port)"
  cat <<EOF_WARP_STEPS
Как правильно подключить WARP к x-ui:

1. Установи и подключи WARP в этом меню.
2. Убедись, что локальный SOCKS5 работает на 127.0.0.1:${port}.
3. В панели x-ui создай outbound с tag "warp" и protocol "socks".
4. В routing rules направь нужные домены через outboundTag "warp".
5. Перезапусти Xray в панели.

Скрипт больше не правит config.json x-ui автоматически, чтобы панель не затирала изменения.
EOF_WARP_STEPS
}
...
warp_menu() {
  while true; do
    show_header
    echo -e "${C_GRAY}Раздел WARP: установка через warp-cli в локальный SOCKS5 proxy mode, запуск, остановка, перевыпуск регистрации, оптимизация, watchdog и готовые JSON-шаблоны для ручной вставки в x-ui.${C_RESET}"
    echo
    ...
    menu_line "16" "🧩" "$C_GREEN" "Показать outbound JSON для x-ui"
    menu_line "17" "🧭" "$C_BLUE" "Показать routing JSON для x-ui"
    menu_line "18" "📋" "$C_YELLOW" "Показать инструкцию по ручному добавлению в x-ui"
    menu_line "0" "↩" "$C_GRAY" "Назад"
    ...
    case "$c" in
      ...
      16) show_warp_xui_outbound_snippet; pause ;;
      17) show_warp_xui_routing_snippets; pause ;;
      18) show_warp_xui_manual_steps; pause ;;
      0) break ;;
      *) echo -e "${C_RED}Неверный выбор.${C_RESET}"; pause ;;
    esac
  done
...
update_menu() {
  while true; do
    show_header
    load_env
    echo -e "${C_GRAY}Обновление менеджера: GitHub raw / GitHub blob / Яндекс Диск, сохранённый URL, просмотр версии и откат к предыдущей копии.${C_RESET}"
...
main_menu() {
  ...
  while true; do
    ...
    menu_line "3" "🛡" "$C_CYAN" "WARP — установка, SOCKS proxy mode, x-ui шаблоны"
