...
load_env() {
  ...
  SOCKS5_LOGIN="${SOCKS5_LOGIN:-proxy}"
...
validate_socks5_credential() {
  ...
  if ! LC_ALL=C grep -Eq '^[A-Za-z0-9._@%+=,!?-]+$' <<<"$value"; then
    echo -e "${C_RED}${field_name} содержит недопустимые символы. Разрешены латинские буквы, цифры и символы . _ @ % + = , ! ? -${C_RESET}"
    return 1
  fi
  return 0
}
...
prompt_socks5_settings() {
  load_env
  local input
  echo -e "${C_DIM}Настройка SOCKS5-прокси. Вручную нужно указать только логин и пароль.${C_RESET}"
  read -r -p "Логин SOCKS5 [${SOCKS5_LOGIN}]: " input || true
  if [[ -n "$input" ]]; then
    SOCKS5_LOGIN="$(strip_socks5_input "$input")"
  else
    SOCKS5_LOGIN="$(strip_socks5_input "$SOCKS5_LOGIN")"
  fi
  read -r -p "Пароль SOCKS5 [${SOCKS5_PASSWORD}]: " input || true
  if [[ -n "$input" ]]; then
    SOCKS5_PASSWORD="$(strip_socks5_input "$input")"
  else
    SOCKS5_PASSWORD="$(strip_socks5_input "$SOCKS5_PASSWORD")"
  fi
  validate_socks5_credential "Логин SOCKS5" "$SOCKS5_LOGIN" || return 1
  validate_socks5_credential "Пароль SOCKS5" "$SOCKS5_PASSWORD" || return 1
  SOCKS5_BIND_ADDR="0.0.0.0"
  save_env
}
...
write_socks5_config() {
  ...
  validate_socks5_credential "Логин SOCKS5" "$SOCKS5_LOGIN" || return 1
  validate_socks5_credential "Пароль SOCKS5" "$SOCKS5_PASSWORD" || return 1
  cat > "$SOCKS5_CFG_FILE" <<EOF_SOCKS
log $LOG_DIR/socks5-proxy.log D
nserver 1.1.1.1
nserver 8.8.8.8
nscache 65536
...
allow ${SOCKS5_LOGIN}
socks -p${SOCKS5_PORT} -i${SOCKS5_BIND_ADDR}
flush
EOF_SOCKS
...
ensure_socks5_firewall() {
  ...
  if [[ "${SOCKS5_AUTO_OPEN_UFW:-true}" == "true" ]]; then
    ufw allow "${port}/tcp" >/dev/null 2>&1 || true
    echo -e "${C_GREEN}Для SOCKS5 открыт порт ${port}/tcp в UFW.${C_RESET}"
  else
    echo -e "${C_YELLOW}UFW активен. Не забудь открыть порт ${port}/tcp вручную.${C_RESET}"
  fi
}
...
socks5_local_check() {
  python3 - "$SOCKS5_PORT" "$SOCKS5_LOGIN" "$SOCKS5_PASSWORD" <<'EOF_PY_SOCKS'
import socket
import sys

port = int(sys.argv[1])
login = sys.argv[2].encode("utf-8")
...
try:
    with socket.create_connection(("127.0.0.1", port), timeout=3) as sock:
        ...
        data = sock.recv(2)
        if data != bytes((0x05, 0x02)):
            raise SystemExit(1)
        if len(login) > 255 or len(password) > 255:
            raise SystemExit(1)
        sock.sendall(bytes((0x01, len(login))) + login + bytes((len(password),)) + password)
        auth = sock.recv(2)
        if auth != bytes((0x01, 0x00)):
            raise SystemExit(1)
except Exception:
    raise SystemExit(1)
raise SystemExit(0)
...
install_socks5_proxy() {
  ...
  local current_used selected_port service_name
  ...
  if [[ -z "$SOCKS5_PORT" || ( -n "$current_used" && ! "$current_used" =~ 3proxy ) ]]; then
    ...
    if [[ -z "$selected_port" ]]; then
      echo -e "${C_RED}Не удалось автоматически подобрать свободный порт для SOCKS5.${C_RESET}"
      return 1
    fi
  ...
  SOCKS5_BIND_ADDR="0.0.0.0"
  save_env
  echo -e "${C_CYAN}Устанавливаю SOCKS5-прокси...${C_RESET}"
  write_socks5_config || return 1
  write_systemd_unit "$SOCKS5_COMP"
  systemd_reload
  ensure_socks5_firewall
  service_name="$(service_name_for "$SOCKS5_COMP")"
  run_cmd systemctl enable "$service_name"
  if ! systemctl restart "$service_name" >/dev/null 2>&1; then
    systemctl start "$service_name" >/dev/null 2>&1 || true
  fi
  sleep 2
  if safe_is_active "$service_name" | grep -q '^active$' && socks5_local_check; then
    echo -e "${C_GREEN}SOCKS5-прокси установлен и запущен.${C_RESET}"
    echo -e "${C_GREEN}Автоматически выбран порт ${SOCKS5_PORT}, адрес привязки ${SOCKS5_BIND_ADDR}.${C_RESET}"
    show_socks5_connection_info
    return 0
  fi
  echo -e "${C_RED}SOCKS5-прокси установлен, но сервис не прошёл SOCKS5-проверку.${C_RESET}"
  journalctl -u "$service_name" -n 20 --no-pager 2>/dev/null || true
  return 1
}

remove_socks5_proxy() {
  echo -e "${C_YELLOW}Будет удалён SOCKS5-прокси. Продолжить? [y/N]${C_RESET}"
  read -r ans
  [[ "${ans,,}" != "y" ]] && return 0
  remove_component "$SOCKS5_COMP"
  rm -f "$SOCKS5_CFG_FILE"
  rm -rf /usr/local/3proxy "$SOCKS5_SRC_DIR"
  echo -e "${C_GREEN}SOCKS5-прокси удалён.${C_RESET}"
}
...
show_socks5_connection_info() {
  ...
  echo "URL: socks5://${SOCKS5_LOGIN}:${SOCKS5_PASSWORD}@${server_ip:-<IP_СЕРВЕРА>}:${SOCKS5_PORT}"
}

status_socks5_proxy() {
  if ! have_socks5_proxy; then
    echo "SOCKS5-прокси не установлен."
    return 0
  fi
...
socks5_menu() {
  while true; do
    show_header
    echo -e "${C_GRAY}Раздел SOCKS5: автоматическая установка 3proxy из исходников, авто-выбор порта и IP, ручной ввод только логина и пароля.${C_RESET}"
    echo
    menu_line "1" "🧦" "$C_GREEN" "Установить / обновить SOCKS5-прокси"
    menu_line "2" "⚙️" "$C_YELLOW" "Изменить логин и пароль SOCKS5"
    menu_line "3" "▶️" "$C_GREEN" "Запустить SOCKS5-прокси"
    menu_line "4" "⏹" "$C_RED" "Остановить SOCKS5-прокси"
    menu_line "5" "🔄" "$C_YELLOW" "Перезапустить SOCKS5-прокси"
    menu_line "6" "📊" "$C_BLUE" "Статус SOCKS5-прокси"
    menu_line "7" "🔐" "$C_CYAN" "Показать данные для подключения"
    menu_line "8" "🗑" "$C_RED" "Удалить SOCKS5-прокси"
    menu_line "0" "↩" "$C_GRAY" "Назад"
    read -r -p $'\033[36mВыбери:\033[0m ' c
    case "$c" in
      1) install_socks5_proxy; pause ;;
      2) prompt_socks5_settings && write_socks5_config && write_systemd_unit "$SOCKS5_COMP" && systemd_reload && run_cmd systemctl restart "$(service_name_for "$SOCKS5_COMP")"; echo -e "${C_GREEN}Логин и пароль SOCKS5 сохранены.${C_RESET}"; pause ;;
      3) start_component "$SOCKS5_COMP"; pause ;;
      4) stop_component "$SOCKS5_COMP"; pause ;;
      5) run_cmd systemctl restart "$(service_name_for "$SOCKS5_COMP")"; echo "SOCKS5-прокси перезапущен."; pause ;;
...
show_header() {  load_env
  ...
  echo -e "🧦 Пакет SOCKS5 (3proxy): $(fmt_status "$socks5_pkg")"
  echo -e "🧦 Сервис SOCKS5: $(fmt_status "$socks5_svc")"
...
EOF_SVC
  ...
  if [[ "$comp" == "$SOCKS5_COMP" ]]; then
    cat > "$service" <<EOF_SVC
[Unit]
Description=VPN-инструменты - SOCKS5 proxy (3proxy)
After=network-online.target
Wants=network-online.target
...
show_logs_menu() {
  while true; do
    ...
    menu_line "8" "📘" "$C_BLUE" "Показать journal SOCKS5-прокси"
  ...
  while true; do
    ...
    case "$c" in
      1)
        ...
        echo -e "Сервис SOCKS5: $(fmt_status "$(safe_is_active "$(service_name_for "$SOCKS5_COMP")")")"
...
main_menu() {
  ...
  while true; do
    show_header
    echo -e "${C_GRAY}Меню ниже управляет установкой WARP, SOCKS5 и MTProto Proxy, watchdog-сервисами, отчётами, Telegram-ботом, логами и резервными копиями.${C_RESET}"
    echo
    ...
    menu_line "4" "🧦" "$C_CYAN" "SOCKS5-прокси — автоустановка и логин/пароль"
