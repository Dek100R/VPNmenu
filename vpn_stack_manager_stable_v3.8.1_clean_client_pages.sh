...
APP_VERSION="3.8.1-clean-client-pages"
...
ensure_happ_shortlink_infra() {
  ...
  if [[ -f "$cert_file" && -f "$key_file" ]]; then
    # Единый HTTPS-блок для клиентских страниц:
    # /r     — белый список Happ
    # /start — инструкция для клиента, если страница уже создана
    write_shared_client_pages_nginx_conf "$domain" "$port" "$cert_file" "$key_file" "" "$html_file"
  fi
...
shared_client_start_page_file() {
  local domain="$1" port="$2"
  echo "/var/www/html/vpn-start-${domain}-${port}.html"
}

disable_legacy_client_page_nginx_links() {
  local domain="$1" port="$2"
  # Убираем только старые конфиги, которые создавал этот менеджер и которые могут конфликтовать
  # с единым server-блоком на том же домене и порту.
  rm -f /etc/nginx/sites-enabled/happ-shortlink-8444 2>/dev/null || true
  rm -f "/etc/nginx/sites-enabled/vpn-client-start-${domain}-${port}" 2>/dev/null || true
  rm -f "/etc/nginx/sites-enabled/vpn-client-pages-${domain}-${port}.conf" 2>/dev/null || true
}

write_shared_client_pages_nginx_conf() {
  local domain="$1" port="$2" cert_file="$3" key_file="$4" start_page="${5:-}" whitelist_page="${6:-/var/www/html/routing.html}"
  local conf="/etc/nginx/sites-available/vpn-client-pages-${domain}-${port}.conf"
  local start_default

  start_default="$(shared_client_start_page_file "$domain" "$port")"
  if [[ -z "$start_page" && -f "$start_default" ]]; then
    start_page="$start_default"
  fi

  mkdir -p /etc/nginx/sites-available /etc/nginx/sites-enabled /var/www/html
  disable_legacy_client_page_nginx_links "$domain" "$port"

  cat > "$conf" <<EOF_NGX_SHARED
server {
    ...
    location = /r {
        root $(dirname "$whitelist_page");
        try_files /$(basename "$whitelist_page") =404;
        default_type text/html;
    }
EOF_NGX_SHARED

  if [[ -n "$start_page" ]]; then
    cat >> "$conf" <<EOF_NGX_START
...
EOF_NGX_END
  ...
  ln -sf "$conf" "/etc/nginx/sites-enabled/vpn-client-pages-${domain}-${port}.conf"
}

setup_client_start_page() {
  clear
  echo -e "${C_CYAN}${C_BOLD}Клиентская страница /start${C_RESET}"
  echo -e "${C_GRAY}Создаёт понятную страницу для клиента и аккуратно объединяет её с белым списком Happ на одном HTTPS-порту.${C_RESET}"
  echo -e "${C_DIM}Итог: https://домен:8444/start и https://домен:8444/r живут в одном Nginx server-блоке.${C_RESET}"
  echo
  ...
  local default_domain domain page_port sub_link whitelist_link title page safe_title cert_file key_file
  default_domain=""
  if [[ -d /etc/letsencrypt/live ]]; then
    default_domain="$(find /etc/letsencrypt/live -mindepth 1 -maxdepth 1 -type d -printf '%f\n' 2>/dev/null | grep -v '^README$' | head -n1 || true)"
  fi

  read -r -p "Домен для клиентской страницы${default_domain:+ [$default_domain]}: " domain || true
  domain="${domain:-$default_domain}"
  [[ -n "$domain" ]] || { echo -e "${C_RED}Домен не указан.${C_RESET}"; return 1; }

  read -r -p "HTTPS-порт для /start и /r [8444]: " page_port || true
  ...
  whitelist_link="https://${domain}:${page_port}/r"
  read -r -p "Готовая SUB-ссылка VPN для кнопки 'Добавить VPN' [можно пусто]: " sub_link || true
  read -r -p "Ссылка белого списка Happ [$whitelist_link]: " _wl || true
  whitelist_link="${_wl:-$whitelist_link}"
  read -r -p "Название сервиса [VPN]: " title || true
  title="${title:-VPN}"
  safe_title="$(python3 -c 'import html,sys; print(html.escape(sys.argv[1]))' "$title")"

  ensure_happ_shortlink_placeholder_page "/var/www/html/routing.html"
  page="$(shared_client_start_page_file "$domain" "$page_port")"
...
<head>
  ...
  <style>
    *{box-sizing:border-box}body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;background:radial-gradient(circle at top,#183457 0,#07101e 46%,#020409 100%);color:#eef6ff;display:flex;min-height:100vh;align-items:center;justify-content:center;padding:22px}.box{max-width:620px;width:100%;background:rgba(13,22,41,.94);border:1px solid rgba(79,171,255,.22);border-radius:24px;padding:28px;box-shadow:0 20px 70px rgba(0,0,0,.42)}h1{margin:0 0 10px;font-size:28px}.muted{color:#b8c8e8;line-height:1.55;margin:0 0 18px}.btn{display:block;text-align:center;text-decoration:none;color:white;background:#2f7cff;border-radius:16px;padding:16px 18px;margin:12px 0;font-weight:800}.btn2{background:#18a058}.btn3{background:#3b465f}.card{background:#0b1428;border:1px solid rgba(255,255,255,.07);border-radius:16px;padding:15px 17px;color:#d5e2ff;line-height:1.55;margin:12px 0}.small{font-size:13px;color:#94a5c9;margin-top:14px;word-break:break-word}.warn{color:#ffd27a}
  </style>
</head>
<body>
  <div class="box">
    <h1>${safe_title}</h1>
    <p class="muted">Страница подключения. Выполни шаги сверху вниз: установи Happ, добавь VPN-подписку, затем добавь белый список.</p>
...
EOF_HTML
  ...
  if [[ -n "$sub_link" ]]; then
    ...
    <div class="card">Если кнопка не открылась автоматически: скопируй SUB-ссылку у администратора и добавь её в Happ как подписку.</div>
EOF_HTML
  else
    cat >> "$page" <<'EOF_HTML'
    <div class="card"><b>2. Добавить VPN-подписку</b><br>Скопируй SUB-ссылку, которую выдал администратор, и добавь её в Happ как подписку.</div>
EOF_HTML
  fi

  cat >> "$page" <<EOF_HTML
    <a class="btn btn2" href="$whitelist_link">3. Добавить белый список Happ</a>
    <div class="card">
      <b>Проверка:</b><br>
      Android: после добавления нажми «Подключить».<br>
      iPhone: используй только HTTPS SUB-ссылку. HTTP на iPhone блокируется.<br>
      <span class="warn">Если белый список не открылся автоматически, нажми кнопку ещё раз.</span>
    </div>
    <div class="small">Страница подключения: https://${domain}:${page_port}/start<br>Белый список: https://${domain}:${page_port}/r</div>
  </div>
</body>
</html>
EOF_HTML

  chmod 644 "$page" >/dev/null 2>&1 || true

  write_shared_client_pages_nginx_conf "$domain" "$page_port" "$cert_file" "$key_file" "$page" "/var/www/html/routing.html"

  if ! nginx -t; then
    echo -e "${C_RED}Nginx не прошёл проверку. Конфиг: /etc/nginx/sites-available/vpn-client-pages-${domain}-${page_port}.conf${C_RESET}"
    return 1
  fi
  systemctl restart nginx
  ...
  echo
  echo -e "${C_GREEN}${C_BOLD}Готово.${C_RESET}"
  echo -e "${C_GREEN}Клиентская страница:${C_RESET} https://${domain}:${page_port}/start"
  echo -e "${C_GREEN}Белый список Happ:${C_RESET} https://${domain}:${page_port}/r"
  echo -e "${C_DIM}Обе страницы находятся в одном Nginx server-блоке, без дублей на одном порту.${C_RESET}"
}
...
settings_menu_clean() {
  while true; do
    show_header
    echo -e "${C_GRAY}Дополнительные настройки: Telegram, обновление менеджера, служебные пути и удаление компонентов.${C_RESET}"
    echo
    ...
    menu_line "5" "🗑" "$C_RED" "Удаление компонентов"
    menu_line "0" "↩" "$C_GRAY" "Назад"
    read -r -p $'\033[36mВыбери:\033[0m ' c
    case "$c" in
      1) telegram_menu ;;
