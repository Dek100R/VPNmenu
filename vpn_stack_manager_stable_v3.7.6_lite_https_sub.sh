...
APP_VERSION="3.7.6-lite-https-sub"
...
main_menu() {
  ...
  while true; do
    show_header
    echo -e "${C_GRAY}Главное меню: WARP/x-ui, HTTPS-подписки, белые списки Happ, Telegram, автоматизация, диагностика и обслуживание.${C_RESET}"
    echo
    menu_line "1" "🚀" "$C_GREEN" "Сценарии и быстрый старт"
    menu_line "2" "🛡" "$C_CYAN" "WARP и интеграция в x-ui"
    menu_line "3" "🤖" "$C_BLUE" "Telegram и уведомления"
    menu_line "4" "🩺" "$C_YELLOW" "Мониторинг и автоматизация"
    menu_line "5" "🔎" "$C_BLUE" "Диагностика и журналы"
    menu_line "6" "🗂" "$C_MAGENTA" "Обслуживание: backup, update, remove"
    menu_line "7" "✅" "$C_GREEN" "Установить / обновить белый список Happ"
    menu_line "8" "🔐" "$C_CYAN" "Настроить HTTPS-подписку 3X-UI для Happ/iPhone"
    ...
    case "$choice" in
      ...
      5) diagnostics_menu ;;
      6) maintenance_menu ;;
      7) update_happ_whitelist_page; pause ;;
      8) setup_xui_https_subscription; pause ;;
      0) exit 0 ;;
      *) echo -e "${C_RED}Неверный выбор.${C_RESET}"; pause ;;
    esac
  done
