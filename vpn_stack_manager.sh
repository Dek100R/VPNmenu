...
prompt_socks5_settings() {
  ...
  [[ -n "$input" ]] && SOCKS5_LOGIN="$input"
  read -r -p "Пароль SOCKS5 [${SOCKS5_PASSWORD}]: " input || true
  [[ -n "$input" ]] && SOCKS5_PASSWORD="$input"
  if [[ -z "$SOCKS5_LOGIN" || -z "$SOCKS5_PASSWORD" ]]; then
    echo -e "${C_RED}Логин и пароль не должны быть пустыми.${C_RESET}"
    return 1
  fi
  SOCKS5_BIND_ADDR="0.0.0.0"
  save_env
