...
' \
    ...
    "allow ${SOCKS5_LOGIN}" \
    "proxy -n -a -p${SOCKS5_PORT} -i${SOCKS5_BIND_ADDR}" \
    "flush" > "$SOCKS5_CFG_FILE"
...
install_socks5_proxy() {
  ...
  sleep 2
  if safe_is_active "$(service_name_for "$SOCKS5_COMP")" | grep -q '^active$' && socks5_local_check; then
  ...
  echo -e "${C_RED}SOCKS5-прокси установлен, но сервис не прошёл локальную проверку. Проверь статус и лог.${C_RESET}"
  return 1
}
